...
export function labStatus(runtime = null) {
  ...
  return Object.freeze({ banner: BANNER, reviewer: "founder", source_commit: process.env.LUMIS_SOURCE_COMMIT || null, window_live: fixtureLive || freeTextLive, fixture_live: fixtureLive, free_text_live: freeTextLive, code: freeTextLive ? "FOUNDER_FREE_TEXT_READY" : fixtureLive ? "FOUNDER_FIXTURE_WINDOW_READY" : "DICE_AI_DISABLED", provider_calls: 0, persistence_writes: 0, units_charged: 0, prompt_version: PROMPT_VERSION, result_schema: RESULT_SCHEMA, contract_commit: CONTRACT_COMMIT, contract_seal_sha256: CONTRACT_SEAL, technical_evidence_sha256: runtime?.technicalEvidenceSha256 ?? null, founder_window_receipt_sha256: runtime?.receiptSha256 ?? null });
}
...
async function loadJson(url){const r=await fetch(url,{cache:'no-store'});if(!r.ok)throw new Error('BOOTSTRAP_'+url);return r.json()}async function init(){[status,fixtures,options]=await Promise.all(['/api/status','/api/fixtures','/api/options'].map(loadJson));if(!Array.isArray(fixtures)||fixtures.length!==40||!options?.planets?.length||!options?.signs?.length||!options?.houses?.length)throw new Error('BOOTSTRAP_INVALID');$('state').textContent='free_text='+(status.free_text_live?'READY':'DISABLED')+' · fixtures='+(status.fixture_live?'READY':'DISABLED')+' · persistence=0 · units=0\\ncontract='+status.contract_commit+'\\nseal='+status.contract_seal_sha256+'\\nschema='+status.result_schema;$('fixture').innerHTML=fixtures.map(f=>'<option value="'+esc(f.fixture_id)+'">'+esc(f.authoring_id+' · '+f.language+' · '+f.question)+'</option>').join('');$('planet').innerHTML=optionHtml(options.planets);$('sign').innerHTML=optionHtml(options.signs);$('house').innerHTML=optionHtml(options.houses);showFixture();setMode('free_text')}
function selectedFixture(){return fixtures.find(x=>x.fixture_id===$('fixture').value)}function showFixture(){const f=selectedFixture();$('fixtureInfo').textContent=f?'fixture_id='+f.fixture_id+' · language='+f.language:''}function setMode(value){mode=value;$('freeTextWrap').classList.toggle('hidden',mode!=='free_text');$('fixtureWrap').classList.toggle('hidden',mode!=='fixture');$('run').disabled=mode==='free_text'?!status.free_text_live:!status.fixture_live;latestMetadata=null;$('classification').textContent='';$('response').innerHTML='<p class="muted">Validated reading appears here.</p>'}
function renderPresentation(p){if(!p)return '<p>Request stopped safely.</p>';if(p.kind!=='reading')return '<p>'+esc(p.message)+'</p>';return '<p class="opening">'+esc(p.opening)+'</p>'+p.sections.map(s=>'<h3>'+esc(s.heading)+'</h3><p>'+esc(s.body)+'</p>').join('')}
let latestMetadata=null;document.querySelectorAll('input[name="mode"]').forEach(x=>x.addEventListener('change',()=>setMode(x.value)));$('fixture').addEventListener('change',()=>{latestMetadata=null;showFixture()});$('run').onclick=async()=>{latestMetadata=null;$('classification').textContent='';$('response').innerHTML='<p>Reading…</p>';const landing={planet_id:$('planet').value,sign_id:$('sign').value,house_id:$('house').value};const body=mode==='free_text'?{question:$('question').value,...landing}:{fixture_id:$('fixture').value,...landing};const endpoint=mode==='free_text'?'/api/run/free-text':'/api/run/fixture';const r=await fetch(endpoint,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});const data=await r.json();latestMetadata=data.metadata||null;const c=data.classification;$('classification').textContent=c?(c.accepted?'route='+c.route+' · shape='+c.shape+' · language='+c.language:'stopped='+c.code+' · language='+c.language):'';$('response').dataset.failureClass=typeof data.redacted_failure_code==='string'?data.redacted_failure_code:'';$('response').innerHTML=renderPresentation(data.presentation)};
