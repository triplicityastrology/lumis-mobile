...
// --- Shape 6: top-level content_filters ARRAY (Responses API) with per-entry source_type ---
test("parses top-level content_filters array with per-entry source_type (jailbreak)", () => {
  const hits = summarizeAzureContentFilter({
    content_filters: [
      { source_type: "prompt", content_filter_results: { jailbreak: { detected: true, filtered: true } } },
    ],
  });
  assertClosed(hits);
  assert.equal(hits.length, 1);
  assert.equal(hits[0].source_type, "prompt");
  assert.equal(hits[0].category, "jailbreak");
  assert.equal(hits[0].detected, true);
  assert.equal(hits[0].filtered, true);
  assert.equal(hits[0].blocked, true);
  assert.equal(hits[0].severity, null);
});

test("content_filters array derives completion vs prompt per entry and defaults missing to prompt", () => {
  const hits = summarizeAzureContentFilter({
    content_filters: [
      { source_type: "completion", content_filter_results: { violence: { filtered: true, severity: "high" } } },
      { content_filter_results: { self_harm: { filtered: true, severity: "medium" } } }, // no source_type -> prompt
    ],
  });
  assertClosed(hits);
  assert.equal(hits.find((h) => h.category === "violence")?.source_type, "completion");
  assert.equal(hits.find((h) => h.category === "self_harm")?.source_type, "prompt");
});
...
// Capture the narrowed config at top level: control-flow narrowing from the guard above does not
// reach into the nested filterAdapter() closure, so referencing cfg.config there would not type-check.
...
function filterAdapter(json: unknown) {
  ...
  return createLabAzureResponsesAdapter(adapterConfig, fn, () => Date.now());
}
...
test("adapter attaches redacted summary ONLY when LUMIS_LAB_FILTER_DIAGNOSTIC=1", async () => {
  ...
  const diagnostic = on.content_filter_diagnostic;
  if (!diagnostic || diagnostic.length !== 1) {
    throw new Error("expected exactly one redacted content-filter diagnostic entry");
  }

  const firstDiagnostic = diagnostic[0];
  if (!firstDiagnostic) {
    throw new Error("missing redacted content-filter diagnostic entry");
  }

  assertClosed(diagnostic);
  assert.equal(firstDiagnostic.category, "self_harm");
  assert.equal(firstDiagnostic.blocked, true);
