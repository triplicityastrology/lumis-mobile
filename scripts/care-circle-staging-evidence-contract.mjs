...
assert.match(harness, /for \(let index = 1; index <= 6; index \+= 1\)/);
assert.match(harness, /new Set\(pending\.map\([^\n]+relationship_id\)\)\.size === 6/);
assert.match(harness, /pending_projection_no_authority/);
assert.match(harness, /status === "pending_caree_acceptance"/);
assert.match(harness, /accepted_at === null/);
assert.match(harness, /carer_accept_denial/);
assert.match(harness, /carer_decline_denial/);
assert.match(harness, /const acceptance = await Promise\.all/);
assert.match(harness, /capacity_projection_state/);
assert.match(harness, /item\.status === "active"/);
assert.match(harness, /sixth_carer_denial/);
assert.match(harness, /request_id_conflict/);
assert.match(harness, /unrelated_projection_empty/);
assert.match(harness, /participant_deletion_cleanup/);
assert.match(harness, /unrelated_relationship_preserved/);
