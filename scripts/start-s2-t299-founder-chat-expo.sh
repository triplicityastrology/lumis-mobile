...
PORT="${FOUNDER_CHAT_T299_EXPO_PORT:-8173}"
stop() { printf 'STOP_S2_T299_EXPO_%s\n' "$1" >&2; exit 1; }
cd "$ROOT"
...
printf 'S2_T299_FOUNDER_CHAT_EXPO_READY source_sha=%s route=polished-talk-prelogin port=%s mode=local-only\n' "$HEAD" "$PORT"
