...
export const RUNTIME_AUTHORITY_TREE = "666558397b6247ffa54b25ff8ac3f5c64ff5989e";
export const AUTHORIZATION_WINDOW_SECONDS = 900;
export const CLOCK_POLICY = "SIGNED_RECEIPT_ISSUED_AT_PLUS_RELATIVE_WINDOW";

export const STOP = Object.freeze({
  control: "STOP_S2_T287_CONTROL_INVALID",
  source: "STOP_S2_T287_SOURCE_DRIFT",
  git: "STOP_S2_T287_GIT_IDENTITY_INVALID",
  dirty: "STOP_S2_T287_WORKTREE_NOT_CLEAN",
  authorization: "STOP_S2_T287_AUTHORIZATION_REQUIRED",
  ...
  replay: "STOP_S2_T287_DEPLOYMENT_CLAIM_REPLAYED",
  claim: "STOP_S2_T287_DEPLOYMENT_CLAIM_FAILED",
  migration: "STOP_S2_T287_MIGRATION_NOT_AUTHORIZED",
  receipt: "STOP_S2_T287_POST_DEPLOY_RECEIPT_INVALID",
  probe: "STOP_S2_T287_DISABLED_PROBE_INVALID",
  rollback: "STOP_S2_T287_ROLLBACK_RECEIPT_INVALID",
});
...
export function validateGitIdentity(identity) {
  exact(identity, ["source_commit", "source_tree", "clean"], STOP.git);
  if (!COMMIT.test(identity.source_commit) || !COMMIT.test(identity.source_tree)) stop(STOP.git);
  if (identity.clean !== true) stop(STOP.dirty);
  return identity;
}

export function validateControl(control) {
  exact(control, ["schema", "status", "authorization_scope", "project_ref", "function_name", "release_parent_commit", "runtime_authority_commit", "runtime_authority_tree", "runtime_package_sha256", "bindings", "runtime_proof", "configuration_names", "kill_switch", "traffic_switch", "provider_calls_authorized", "model_invocations_authorized", "disabled_probes", "migration_boundary", "normal_chat_binding", "authorization", "rollback", "remote_calls_in_default_mode", "normal_chat_authority", "azure_traffic_authority"], STOP.control);
  if (control.schema !== "s2_t287_dice_default_off_deployment_control_v1" || control.status !== WAITING || control.authorization_scope !== SCOPE || control.project_ref !== PROJECT_REF || control.function_name !== FUNCTION_NAME) stop(STOP.control);
  if (control.release_parent_commit !== RELEASE_PARENT_COMMIT || control.runtime_authority_commit !== RUNTIME_AUTHORITY_COMMIT || control.runtime_authority_tree !== RUNTIME_AUTHORITY_TREE || !SHA256.test(control.runtime_package_sha256)) stop(STOP.package);
  ...
  for (const binding of Object.values(control.bindings)) {
    ...
    if (typeof binding.path !== "string" || !SHA256.test(binding.sha256)) stop(STOP.control);
  }
  exact(control.runtime_proof, ["deno_check", "edge_eszip_bundle", "edge_eszip_sha256", "import_graph_sha256", "disabled_before_json_parse_and_client_construction", "disabled_probes_passed", "provider_calls", "model_invocations", "remote_calls"], STOP.control);
  if (control.runtime_proof.deno_check !== "passed" || control.runtime_proof.edge_eszip_bundle !== "passed" || !SHA256.test(control.runtime_proof.edge_eszip_sha256) || !SHA256.test(control.runtime_proof.import_graph_sha256) || control.runtime_proof.disabled_before_json_parse_and_client_construction !== true || control.runtime_proof.disabled_probes_passed !== 4 || control.runtime_proof.provider_calls !== 0 || control.runtime_proof.model_invocations !== 0 || control.runtime_proof.remote_calls !== 0) stop(STOP.control);
...
export async function verifyPinnedSources(control, root = process.cwd()) {
  ...
  for (const observed of [control.migration_boundary, { path: control.normal_chat_binding.entry_path, sha256: control.normal_chat_binding.current_sha256 }]) {
    let bytes;
    try { bytes = await readFile(`${root}/${observed.path}`); } catch { stop(STOP.source); }
    if (sha256(bytes) !== observed.sha256) stop(STOP.source);
  }
  return true;
}

export function validatePackageSeal(seal) {
  exact(seal, ["schema", "base_commit", "runtime_package_sha256", "authorization_package_sha256", "authorization_clock", "migration_0039_application_authorized", "provider_calls_authorized", "model_invocations_authorized", "files"], STOP.package);
  if (seal.schema !== "s2_t287_dice_v4_deployment_package_seal_v1" || seal.base_commit !== RELEASE_PARENT_COMMIT || !SHA256.test(seal.runtime_package_sha256) || !SHA256.test(seal.authorization_package_sha256)) stop(STOP.package);
  exact(seal.authorization_clock, ["policy", "window_seconds"], STOP.package);
  if (seal.authorization_clock.policy !== CLOCK_POLICY || seal.authorization_clock.window_seconds !== AUTHORIZATION_WINDOW_SECONDS || seal.migration_0039_application_authorized !== false || seal.provider_calls_authorized !== 0 || seal.model_invocations_authorized !== 0) stop(STOP.package);
  if (!isRecord(seal.files) || Object.keys(seal.files).length < 10 || Object.values(seal.files).some((digest) => !SHA256.test(digest))) stop(STOP.package);
  const canonical = Object.entries(seal.files).sort(([left], [right]) => left.localeCompare(right)).map(([path, digest]) => `${path}:${digest}`).join("\n");
  if (sha256(`${canonical}\n`) !== seal.authorization_package_sha256) stop(STOP.package);
  return seal;
}

export async function verifyAuthorizationPackage(seal, root = process.cwd()) {
  validatePackageSeal(seal);
  for (const [path, expected] of Object.entries(seal.files)) {
    let bytes;
    try { bytes = await readFile(`${root}/${path}`); } catch { stop(STOP.source); }
    if (sha256(bytes) !== expected) stop(STOP.source);
  }
  return true;
}
...
export function validateAuthorizationReceipt(receipt, control, seal, request, identity, microsoftPublicKeyPem, now = Date.now()) {
  ...
  if (receipt.source_commit !== identity.source_commit || receipt.source_tree !== identity.source_tree || receipt.runtime_package_sha256 !== control.runtime_package_sha256 || receipt.authorization_package_sha256 !== seal.authorization_package_sha256) stop(STOP.package);
  ...
  if (receipt.authorization_window_seconds !== control.authorization.window_seconds || receipt.clock_policy !== control.authorization.clock_policy || receipt.kill_switch_required !== false || receipt.traffic_switch_required !== false || receipt.provider_calls_authorized !== 0 || receipt.model_invocations_authorized !== 0 || !sameArray(receipt.disabled_probes, control.disabled_probes) || receipt.migration_application_authorized !== false || receipt.migration_required_authorization_scope !== control.migration_boundary.required_authorization_scope) stop(STOP.authorizationShape);
...
export async function claimAuthorization(authorization, ledgerPath) {
  if (!authorization || !DEPLOYMENT_ID.test(authorization.deploymentId) || !SHA256.test(authorization.authorizationSha256) || !REVISION.test(authorization.rollbackRevision) || !COMMIT.test(authorization.sourceCommit)) stop(STOP.claim);
  ...
  try {
    await handle.writeFile(`${JSON.stringify({ schema: "s2_t287_dice_deployment_claim_v1", deployment_id: authorization.deploymentId, authorization_sha256: authorization.authorizationSha256, source_commit: authorization.sourceCommit, rollback_revision: authorization.rollbackRevision, consumed_once: true })}\n`, { encoding: "utf8" });
  } finally { await handle.close(); }
  return Object.freeze({ status: "CLAIMED", deploymentId: authorization.deploymentId });
}

export function validatePostDeployReceipt(receipt, authorization, control, seal) {
  exact(receipt, ["schema", "project_ref", "function_name", "deployment_id", "authorization_sha256", "request_sha256", "signing_key_sha256", "source_commit", "source_tree", "runtime_package_sha256", "authorization_package_sha256", "configuration_names_verified", "kill_switch_disabled", "traffic_switch_disabled", "function_version", "rollback_revision", "disabled_probes", "provider_calls", "model_invocations", "normal_chat_unchanged", "migration_applied", "rollback_target", "deployed_at", "credentials_unset"], STOP.receipt);
  if (receipt.schema !== "s2_t287_dice_default_off_deployment_receipt_v1" || receipt.project_ref !== PROJECT_REF || receipt.function_name !== FUNCTION_NAME || receipt.deployment_id !== authorization.deploymentId || receipt.authorization_sha256 !== authorization.authorizationSha256 || !SHA256.test(receipt.request_sha256) || !SHA256.test(receipt.signing_key_sha256) || receipt.source_commit !== authorization.sourceCommit || !COMMIT.test(receipt.source_tree) || receipt.runtime_package_sha256 !== control.runtime_package_sha256 || receipt.authorization_package_sha256 !== seal.authorization_package_sha256) stop(STOP.receipt);
...
export function validateRollbackReceipt(receipt, authorization, control) {
  exact(receipt, ["schema", "project_ref", "function_name", "deployment_id", "source_commit", "function_disabled", "function_removed_or_previous_restored", "restored_revision", "migration_0039_unchanged", "provider_calls", "model_invocations", "normal_chat_unchanged", "credentials_unset"], STOP.rollback);
  if (receipt.schema !== "s2_t287_dice_rollback_receipt_v1" || receipt.project_ref !== PROJECT_REF || receipt.function_name !== FUNCTION_NAME || receipt.deployment_id !== authorization.deploymentId || receipt.source_commit !== authorization.sourceCommit || receipt.function_disabled !== true || receipt.function_removed_or_previous_restored !== true || receipt.restored_revision !== authorization.rollbackRevision || receipt.migration_0039_unchanged !== true || receipt.provider_calls !== 0 || receipt.model_invocations !== 0 || receipt.normal_chat_unchanged !== true || receipt.credentials_unset !== true || control.migration_boundary.application_authorized !== false) stop(STOP.rollback);
  return receipt;
}
