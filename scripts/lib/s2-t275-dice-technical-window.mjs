...
export const STOP = Object.freeze({
  ...
  source: "STOP_S2_T275_SEALED_SOURCE_DRIFT",
  registry: "STOP_S2_T275_TECHNICAL_REGISTRY_INVALID",
  authority: "STOP_S2_T275_AUTHORITY_INVALID",
  replay: "STOP_S2_T275_RUN_AUTHORITY_REPLAYED",
  attempts: "STOP_S2_T275_ATTEMPT_CAP",
  tokens: "STOP_S2_T275_TOKEN_CAP",
  concurrency: "STOP_S2_T275_CONCURRENCY_CAP",
  deadline: "STOP_S2_T275_CASE_DEADLINE",
  unsafe: "STOP_S2_T275_UNSAFE_EVIDENCE",
  provider: "STOP_S2_T275_PROVIDER_KILL_CRITERION",
  disable: "STOP_S2_T275_POST_WINDOW_DISABLE_FAILED",
  evidence: "STOP_S2_T275_EVIDENCE_INVALID",
});
...
export function loadAndValidateControl(root = process.cwd()) {
  const control = JSON.parse(readFileSync(`${root}/config/s2-t275-dice-technical-window.json`, "utf8"));
  ...
  if (control.base_commit !== "776620bfa1bf3c80090313c83e753cb3004db50d" ||
...
export function validateDeploymentReceipt(value, control, now = Date.now()) {
  exact(value, ["schema", "project_ref", "function_name", "deployment_id", "source_commit", "package_sha256", "provider_calls", "disabled_verified", "issued_at", "expires_at"], STOP.deployment);
  const issued = Date.parse(value.issued_at), expires = Date.parse(value.expires_at);
  if (value.schema !== "s2_t275_accepted_default_off_deployment_receipt_v1" || value.project_ref !== "bmqhwofmdgebpcihjlnb" ||
      value.function_name !== "dice-synthetic" || !/^dice-deploy-[a-z0-9]{16,40}$/.test(value.deployment_id) ||
      value.source_commit !== control.base_commit || value.package_sha256 !== control.gateway_package_sha256 || value.provider_calls !== 0 ||
      value.disabled_verified !== true || !Number.isFinite(issued) || !Number.isFinite(expires) || issued > now || expires <= now) stop(STOP.deployment);
  return value;
}

export function validateTrafficAuthority(value, control, deployment, now = Date.now()) {
  exact(value, ["schema", "authorization_scope", "single_use_run_id", "deployment_id", "source_commit", "package_sha256", "technical_cases", "founder_cases", "attempt_cap", "cost_ceiling_usd", "issued_at", "valid_until", "microsoft_review", "gateway_authorization"], STOP.traffic);
  const issued = Date.parse(value.issued_at), expires = Date.parse(value.valid_until);
  if (value.schema !== "s2_t275_dice_technical_traffic_authorization_v1" || value.authorization_scope !== "technical_80_only" || !RUN.test(value.single_use_run_id) ||
      value.deployment_id !== deployment.deployment_id || value.source_commit !== control.base_commit || value.package_sha256 !== control.gateway_package_sha256 ||
      value.technical_cases !== 80 || value.founder_cases !== 0 || value.attempt_cap !== 160 || value.cost_ceiling_usd !== 0.128 ||
...
export async function runTechnicalWindow({ gateway, deploymentReceipt, trafficAuthority, root = process.cwd(), claimRun = async () => true, onRecord = async () => {} }) {
  const { control, registry } = loadAndValidateControl(root);
  const deployment = validateDeploymentReceipt(deploymentReceipt, control);
  const authority = validateTrafficAuthority(trafficAuthority, control, deployment);
  ...
  return Object.freeze({
    schema: "s2_t275_dice_technical_evidence_package_v1",
...
export function validateEvidencePackage(value) {
  const keys = ["schema", "run_id", "source_commit", "package_sha256", "technical_case_count", "founder_case_count", "language", "attempt_total", "concurrency_peak", "cost_ceiling_usd", "provider_disabled_verified", "effects", "records"];
  exact(value, keys, STOP.evidence);
  if (value.schema !== "s2_t275_dice_technical_evidence_package_v1" || !RUN.test(value.run_id) || value.source_commit !== "776620bfa1bf3c80090313c83e753cb3004db50d" ||
      value.package_sha256 !== "3ccc7551fd945b4ca4c3aaeaa7b8f9efd61f29b56e8ebe3c69ea9f5c5aaae8ba" || value.technical_case_count !== 80 || value.founder_case_count !== 0 ||
