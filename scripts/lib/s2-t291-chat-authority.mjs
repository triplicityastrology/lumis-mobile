...
export const DICE_ACCEPTANCE_SCHEMA = "lumis_dice_technical_window_80_accepted_evidence_v4";
export const DICE_DEPLOYMENT_AUTHORIZATION_SCHEMA = "lumis_dice_default_off_function_deployment_authorization_v4";
export const DICE_DEPLOYMENT_SCHEMA = "lumis_dice_default_off_function_deployment_receipt_v4";
export const DICE_RUNTIME_PACKAGE_SHA256 = "be911dd5f335217b4d00bbce34f0bd27cd9fcdc7c50152b4406b3b3058528457";
export const DICE_TECHNICAL_SCHEMA = "lumis_dice_technical_window_80_evidence_v4";
...
export function validateDiceEvidence(value, acceptedSha256) {
  exact(value, ["schema", "review_decision", "deployment_receipt", "technical_window", "accepted_at"], "STOP_S2_T291_DICE_EVIDENCE_FIELDS");
  exact(value.deployment_receipt, ["schema", "authorization_schema", "source_commit", "runtime_package_sha256", "disabled_probes", "provider_calls", "model_invocations", "migration_applied", "post_deploy_disabled"], "STOP_S2_T291_DICE_DEPLOYMENT_FIELDS");
  exact(value.deployment_receipt.disabled_probes, ["unknown_fixture", "free_form_body", "normal_mobile_body", "allow_listed_fixture"], "STOP_S2_T291_DICE_PROBE_FIELDS");
  exact(value.technical_window, ["schema", "authority", "evidence_package_sha256", "logical_total", "en", "zh_hant", "attempt_total", "max_attempts", "input_token_limit", "output_token_limit", "concurrency_limit", "shared_deadline_ms", "cost_ceiling_usd", "provider_disabled_verified", "finally_disabled", "post_window_disabled_proof_sha256", "founder_cases_run", "persistence_writes", "units_charged"], "STOP_S2_T291_DICE_TECHNICAL_FIELDS");
  ...
  assert.equal(deployment.runtime_package_sha256, DICE_RUNTIME_PACKAGE_SHA256);
  assert.ok(Object.values(deployment.disabled_probes).every((code) => code === "DICE_AI_DISABLED"));
  assert.deepEqual([deployment.provider_calls, deployment.model_invocations, deployment.migration_applied, deployment.post_deploy_disabled], [0, 0, false, true]);
  ...
  assert.deepEqual([technical.input_token_limit, technical.output_token_limit, technical.concurrency_limit, technical.shared_deadline_ms, technical.cost_ceiling_usd], [800, 300, 2, 12000, 0.128]);
  assert.deepEqual([technical.provider_disabled_verified, technical.finally_disabled, technical.founder_cases_run, technical.persistence_writes, technical.units_charged], [true, true, 0, 0, 0]);
  assert.match(technical.post_window_disabled_proof_sha256, SHA);
...
function validateCommon(value, manifest, head, keys, schema, authority, now = Date.now()) {
  exact(value, keys, "STOP_S2_T291_AUTHORIZATION_FIELDS");
  ...
  assert.ok(Number.isFinite(issued) && Number.isFinite(expires) && issued <= now && expires > now && expires > issued && expires - issued <= 15 * 60 * 1000);
}
...
export function validateDeploymentRequest(value, manifest, head, now) {
  ...
  validateCommon(value, manifest, head, keys, "s2_t291_chat_default_off_deployment_request_v1", "CHAT_SYNTHETIC_DEFAULT_OFF_DEPLOYMENT_ONLY", now);
...
export function validateMigrationRequest(value, manifest, head, now) {
  ...
  validateCommon(value, manifest, head, keys, "s2_t291_chat_migration_0040_request_v1", "CHAT_SYNTHETIC_AUTHORITY_LEDGER_0040_MIGRATION_ONLY", now);
...
export function validateTrafficRequest(value, manifest, head, now) {
  ...
  validateCommon(value, manifest, head, keys, "s2_t291_chat_synthetic_traffic_request_v1", "CHAT_SYNTHETIC_CLOSED_FIXTURE_WINDOW_ONLY", now);
  assert.equal(value.function_name, FUNCTION_NAME);
  assert.ok(manifest.accepted_dice_evidence_sha256 !== null, "STOP_S2_T291_DICE_EVIDENCE_NOT_COMPILED");
  ...
  assert.deepEqual([value.fixture_count, value.language_counts, value.runtime_request_fields], [60, { en: 30, zh_hant: 30 }, ["fixture_id"]]);
