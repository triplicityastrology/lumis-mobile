...
import { loadAndValidateControl, runTechnicalWindow, sha256, validateEvidencePackage } from "./lib/s2-t279-dice-technical-window.mjs";

const { control, registry } = loadAndValidateControl();
const now = Date.now();
const runId = "dice-tech80-localemulator001";
const deploymentReceipt = {
  schema: "s2_t279_accepted_default_off_deployment_receipt_v1", project_ref: "bmqhwofmdgebpcihjlnb", function_name: "dice-synthetic",
  deployment_id: "dice-deploy-localemulator001", runtime_commit: control.runtime_commit, runtime_package_sha256: control.runtime_package_sha256,
  gateway_package_sha256: control.gateway_package_sha256, provider_calls: 0, model_invocations: 0, disabled_verified: true,
  issued_at: new Date(now - 1000).toISOString(), expires_at: new Date(now + 60000).toISOString(),
};
const migrationReceipt = {
  schema: "s2_t279_accepted_dice_0039_migration_receipt_v1", project_ref: "bmqhwofmdgebpcihjlnb", migration_version: "0039",
  migration_name: "dice_synthetic_authority_ledger", migration_sha256: control.ledger_proof.migration_sha256,
  ledger_contract_commit: control.ledger_proof.commit, local_proof_schema_sha256: control.ledger_proof.schema_sha256,
  local_proof_receipt_sha256: control.ledger_proof.receipt_sha256, applied: true, parity_verified: true, rpc_rls_verified: true,
  metadata_only_verified: true, provider_calls: 0, issued_at: new Date(now - 1000).toISOString(), expires_at: new Date(now + 60000).toISOString(),
};
const trafficAuthority = {
  schema: "s2_t279_dice_technical_traffic_authorization_v1", authorization_scope: "DICE_TECHNICAL_SYNTHETIC_WINDOW_80_ONLY", single_use_run_id: runId,
  deployment_id: deploymentReceipt.deployment_id, migration_version: "0039", runtime_commit: control.runtime_commit,
  runtime_package_sha256: control.runtime_package_sha256, gateway_package_sha256: control.gateway_package_sha256,
  ledger_proof_receipt_sha256: control.ledger_proof.receipt_sha256, technical_cases: 80, language: { en: 40, "zh-Hant": 40 }, founder_cases: 0,
  attempt_cap: 160, concurrency: 2, eligible_retries: 1, shared_deadline_ms: 12000, cost_ceiling_usd: 0.128,
...
const qa = {
  schema: "s2_t279_dice_technical_qa_bundle_v1",
...
mkdirSync(".tmp/s2-t279", { recursive: true });
writeFileSync(".tmp/s2-t279/local-emulator-evidence.json", evidenceJson);
writeFileSync(".tmp/s2-t279/local-emulator-qa-bundle.json", `${JSON.stringify(qa, null, 2)}\n`);
assert.equal(localProtocolAttempts, evidence.attempt_total);
console.log(`S2_T279_LOCAL_EMULATOR_OK cases=80 attempts=${evidence.attempt_total} concurrency=${evidence.concurrency_peak} remote_calls=0 evidence_sha256=${qa.evidence_sha256}`);
