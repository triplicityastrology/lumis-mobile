...
while (( $# > 0 )); do
  case "$1" in
    ...
    *) print -u2 -- "STOP_S2_T277_ARGUMENTS_INVALID"; exit 1 ;;
  esac
done
...
[[ -n "$AUTHORIZATION" && -n "$REQUEST" && -n "$CLAIM_LEDGER" && -n "$MICROSOFT_PUBLIC_KEY" && -n "$RECEIPT_OUTPUT" ]] || { print -u2 -- "STOP_S2_T277_AUTHORIZATION_REQUIRED"; exit 1; }
node scripts/s2-t277-dice-deployment-authorization.mjs \
...
[[ "${LUMIS_T277_RUN_REMOTE_DEPLOYMENT:-}" == "DEFAULT_OFF_DICE_SYNTHETIC_FUNCTION_DEPLOYMENT_ONLY" ]] || { print -u2 -- "STOP_S2_T277_REMOTE_EXECUTION_NOT_ENABLED"; exit 1; }
[[ -n "${SUPABASE_ACCESS_TOKEN:-}" && -n "${SUPABASE_ANON_KEY:-}" ]] || { print -u2 -- "STOP_S2_T277_TRANSIENT_CREDENTIALS_REQUIRED"; exit 1; }

RUNTIME_DIR="$(mktemp -d "${TMPDIR:-/tmp}/lumis-s2-t277.XXXXXX")"
trap 'unset SUPABASE_ACCESS_TOKEN SUPABASE_ANON_KEY LUMIS_T277_RUN_REMOTE_DEPLOYMENT; rm -rf "$RUNTIME_DIR"' EXIT HUP INT TERM

pnpm exec supabase secrets list --project-ref "$EXPECTED_REF" --output json > "$RUNTIME_DIR/secrets.json"
node scripts/s2-t277-remote-deploy-proof.mjs configuration --control=config/s2-t277-dice-deployment-authorization.json --input="$RUNTIME_DIR/secrets.json" >/dev/null
pnpm exec supabase functions list --project-ref "$EXPECTED_REF" --output json > "$RUNTIME_DIR/functions-before.json"
ROLLBACK_REVISION="$(node scripts/s2-t277-remote-deploy-proof.mjs revision --input="$RUNTIME_DIR/functions-before.json")"
...
for index in {1..4}; do
  ...
  node scripts/s2-t277-remote-deploy-proof.mjs probe --status="$STATUS" --input="$RUNTIME_DIR/probe-${index}.json" >/dev/null
done

pnpm exec supabase functions list --project-ref "$EXPECTED_REF" --output json > "$RUNTIME_DIR/functions-after.json"
DEPLOYED_REVISION="$(node scripts/s2-t277-remote-deploy-proof.mjs revision --input="$RUNTIME_DIR/functions-after.json")"
unset SUPABASE_ACCESS_TOKEN SUPABASE_ANON_KEY LUMIS_T277_RUN_REMOTE_DEPLOYMENT
node scripts/s2-t277-remote-deploy-proof.mjs receipt \
  --control=config/s2-t277-dice-deployment-authorization.json \
  --authorization="$AUTHORIZATION" \
...
print -- "S2_T277_DEFAULT_OFF_DEPLOYMENT_RECORDED project=exact_staging function=$FUNCTION_NAME provider_calls=0 model_invocations=0 migration_authorized=false"
