...
ROOT=${0:A:h:h}
cd "$ROOT"
RUNTIME="$ROOT/.runtime/s2-t272-proof"
SERVICE="$RUNTIME/root"
CONTAINER="lumis-s2-t272-edge-proof"
NETWORK="lumis-s2-t272-edge-proof"
IMAGE="public.ecr.aws/supabase/edge-runtime@sha256:a82676277615aee03c4f288cbbbf68dedb5ba8693073e567ab8dbfdd11ba5d45"
...
[[ -x node_modules/.bin/deno && -x node_modules/.bin/supabase ]] || { print -u2 "STOP_S2_T272_PINNED_TOOLS_MISSING"; exit 1; }
docker info >/dev/null 2>&1 || { print -u2 "STOP_S2_T272_DOCKER_UNAVAILABLE"; exit 1; }
docker image inspect "$IMAGE" >/dev/null 2>&1 || { print -u2 "STOP_S2_T272_EDGE_IMAGE_NOT_CACHED_NO_PULL_ALLOWED"; exit 1; }
[[ -z ${SUPABASE_ACCESS_TOKEN:-} && -z ${LUMIS_DICE_AZURE_API_KEY:-} ]] || { print -u2 "STOP_S2_T272_REMOTE_CREDENTIAL_ENV_PRESENT"; exit 1; }

rm -rf "$RUNTIME"
mkdir -p "$SERVICE/supabase" "$SERVICE/packages" "$RUNTIME/edge"
cp package.json pnpm-lock.yaml "$SERVICE/"
...
cp -RL node_modules "$SERVICE/supabase/functions/dice-synthetic/node_modules"
cp -RL node_modules/.pnpm/node_modules/. "$SERVICE/supabase/functions/dice-synthetic/node_modules/"
ln -s supabase/functions/dice-synthetic/node_modules "$SERVICE/node_modules"
...
docker run --rm --network none -v "$SERVICE:/work:ro" -v "$RUNTIME/edge:/runtime" -w /work "$IMAGE" \
...
docker network create --internal --label lumis.task=S2-T272 "$NETWORK" >/dev/null
docker run -d --name "$CONTAINER" --label lumis.task=S2-T272 --network "$NETWORK" \
...
[[ $(docker inspect "$CONTAINER" --format '{{.State.Status}}') == running ]] || { docker logs "$CONTAINER" >&2; print -u2 "STOP_S2_T272_EDGE_RUNTIME_START_FAILED"; exit 1; }
for _ in {1..30}; do
...
docker exec "$CONTAINER" /bin/bash -c 'exec 3<>/dev/tcp/127.0.0.1/9000' >/dev/null 2>&1 || {
  ...
  print -u2 "STOP_S2_T272_EDGE_RUNTIME_NOT_READY"
  exit 1
}
...
for index in {1..4}; do
  ...
  [[ "$http_status" == 503 && "$payload" == '{"error":{"code":"DICE_AI_DISABLED"}}' ]] || { print -u2 "STOP_S2_T272_DISABLED_PROBE_FAILED"; exit 1; }
