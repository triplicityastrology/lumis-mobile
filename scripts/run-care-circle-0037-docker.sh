...
for attempt in {1..6}; do
  docker exec "$CONTAINER" psql -v ON_ERROR_STOP=1 -U "$DB_USER" -d lumis_s2_t148 -c "set role service_role; select public.register_care_pairing_attempt_backend('90000000-0000-4000-8000-000000000001');" >"$RESULT_ROOT/attempt-$attempt.out" 2>"$RESULT_ROOT/attempt-$attempt.err" &
done
wait || true
allowed=0; throttled=0
for attempt in {1..6}; do
  if grep -Fq '"allowed": true' "$RESULT_ROOT/attempt-$attempt.out"; then allowed=$((allowed + 1))
  elif grep -Fq '48004' "$RESULT_ROOT/attempt-$attempt.err"; then throttled=$((throttled + 1))
