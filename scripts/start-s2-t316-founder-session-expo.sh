...
PORT="${FOUNDER_T316_EXPO_PORT:-8186}"
stop() { printf 'STOP_S2_T316_EXPO_%s\n' "$1" >&2; exit 1; }
cd "$ROOT"
...
printf 'S2_T316_FOUNDER_SESSION_READY source_sha=%s port=%s route=founder_tomorrow_session dice_result_surface=ready chat=explicit_reflect_only provider_calls=0 persistence=0 units=0\n' "$HEAD" "$PORT"
