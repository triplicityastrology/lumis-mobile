...
PORT="${FOUNDER_CHAT_WINDOW_WEB_PORT:-8151}"
EXPECTED_BRANCH="codex/s2-t281-chat-final-request"
stop() { printf 'STOP_S2_T281_WEB_%s\n' "$1" >&2; exit 1; }
...
EXPORT_ROOT="$ROOT/.tmp/s2-t281-founder-chat-window-$PORT"
case "$EXPORT_ROOT" in "$ROOT/.tmp/s2-t281-founder-chat-window-"*) ;; *) stop EXPORT_PATH_UNSAFE ;; esac
rm -rf "$EXPORT_ROOT"
cleanup() { rm -rf "$EXPORT_ROOT"; }
trap cleanup EXIT INT TERM

printf 'S2_T281_FOUNDER_CHAT_WINDOW_WEB_READY\nsource_sha=%s\nroute=founder-companion-chat-window\nmode=local_closed_synthetic\nopen=http://localhost:%s\n' "$HEAD" "$PORT"
EXPO_OFFLINE=1 EXPO_PUBLIC_FOUNDER_COMPANION_CHAT=1 EXPO_PUBLIC_FOUNDER_COMPANION_HEAD="$HEAD" EXPO_PUBLIC_FOUNDER_COMPANION_STATE="founder-chat-window-prelogin" \
...
grep -RFl -- "Import accepted Dice evidence" "$JS_ROOT" >/dev/null || stop EVIDENCE_MARKER_MISSING
grep -RFl -- "NO_NORMAL_CHAT_INTEGRATION_AUTHORITY" "$JS_ROOT" >/dev/null || stop AUTHORITY_MARKER_MISSING
printf 'S2_T281_EXPORT_VERIFIED source_sha=%s route=founder-companion-chat-window\nKeep this terminal open; press Ctrl+C to stop.\n' "$HEAD"
