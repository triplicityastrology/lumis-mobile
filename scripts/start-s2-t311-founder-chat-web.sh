...
PORT="${FOUNDER_CHAT_T311_WEB_PORT:-8181}"
stop() { printf 'STOP_S2_T311_WEB_%s\n' "$1" >&2; exit 1; }
cd "$ROOT"
...
EXPORT_ROOT="$ROOT/.tmp/s2-t311-founder-chat-$PORT"
rm -rf "$EXPORT_ROOT"
...
printf 'S2_T311_FOUNDER_CHAT_WEB_READY source_sha=%s route=polished-talk-prelogin open=http://localhost:%s\n' "$HEAD" "$PORT"
