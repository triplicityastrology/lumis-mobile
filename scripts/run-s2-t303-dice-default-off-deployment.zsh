...
[[ -n "$AUTHORIZATION" && -n "$REQUEST" && -n "$CLAIM_LEDGER" && -n "$PUBLIC_KEY" && -n "$RECEIPT_OUTPUT" ]] || { print -u2 -- "STOP_S2_T303_SEPARATE_OPERATIONAL_AUTHORIZATION_REQUIRED"; exit 1; }
...
LUMIS_T298_RUN_REMOTE_DEPLOYMENT="DEFAULT_OFF_DICE_SYNTHETIC_FUNCTION_DEPLOYMENT_ONLY" \
  zsh scripts/run-s2-t298-dice-v4-zero-call-deployment.zsh --execute \
    ...
    --microsoft-public-key "$PUBLIC_KEY" --receipt-output "$RECEIPT_OUTPUT"
