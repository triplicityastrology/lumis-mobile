import type { DiceFounderFreeTextRequest } from "./diceFounderFreeTextGateway";

export function createDiceFounderFreeTextTransport() {
  const relayUrl = parseRelayUrl(process.env.EXPO_PUBLIC_DICE_MOBILE_FREE_TEXT_RELAY_URL);
  ...
  return async (body: DiceFounderFreeTextRequest): Promise<unknown> => {
...
function parseRelayUrl(value: string | undefined): string | null {
  try {
    ...
    if (parsed.protocol !== "http:" || !privateHost || parsed.pathname !== "/dice-free-text" || parsed.search || parsed.hash) return null;
    return parsed.href;
  } catch {
    return null;
  }
