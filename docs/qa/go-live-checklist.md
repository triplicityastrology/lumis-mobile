...
- [ ] Do not ship the current Birth Details success path as functional. It advances through timers, exposes a visible developer success/failure toggle, increments only in-memory `birthDetailChanges`, does not update `profileData`, and does not call the pending PROF-2 transaction. Wire the full-screen progress UI to the real backend result, remove the developer toggle from production, and cancel all timers on Back/unmount so a delayed success cannot reopen.
