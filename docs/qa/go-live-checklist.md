...
- [x] Git `main` tracks `origin/main` with no divergence as independently checked on 2026-07-21. Subsequent QA checklist/preservation-document edits remain intentional local QA changes and must not be swept into Technical commits without review.
- [ ] Run and record:
...
- [x] PM/security decision: leave shared Git history unchanged while the repository remains private and restricted to Rubie/trusted core collaborators. Before any public release, external-developer handoff, or access expansion, reopen the decision and likely rewrite history in a coordinated maintenance window with fresh clones on both Macs.
- [x] PM/security decision: the source website records/session identifiers cannot be invalidated because they are real client records. Mitigation is forward-only: keep them out of repository artifacts, command history, generated docs, screenshots, and QA exports; accept them only through hidden local input when fixture refresh is explicitly required; restrict website/admin access to trusted users.
...
- [ ] Prove end-to-end traceability in staging by injecting failures at Chat, profile persistence, Worker/provider, and external sync. Confirm the same/request-linked IDs are searchable without messages, birth data, tokens, or provider payloads; source contracts alone do not prove operational traces or scheduled execution.
- [ ] Make provider-call compensation recording durable across the failure it is intended to capture. `/profile` currently writes `chart_provider_call_events` back to the same Supabase database after the provider returns and logs-and-continues when that write fails. A database outage that causes onboarding persistence to fail can therefore also lose the compensation event; use a separate durable coordinator/outbox or fail the flow before the paid provider call when the reservation cannot be recorded.
...
- [x] Source migration `0023` makes 30 days a strict ceiling at claim/manual-replay time: expired payloads are atomically redacted, moved to `failed_final`, and rejected with `SYNC_PAYLOAD_EXPIRED`; focused contracts pass.
- [x] Source migration `0024` replaces blacklist redaction with a strict operational allowlist. Completed/expired payloads retain only approved operation/request/session/deletion/source identifiers; unlisted current or future fields are private by default. Focused privacy contracts pass.
- [x] Technical reports migrations `0001–0025`, including strict-retention `0023` and concurrency/allowlist `0024`, have local/remote parity. The privileged hosted expiry/replay/privacy assertions remain open until the secure suite runs.
...
- [x] Source migration `0023` adds append-only provider-attempt rows and changes `provider_calls_24h` to count rows by `observed_at`; focused guardrail and Worker fixtures pass. Technical reports Worker version `def65b99-6a3f-429f-800a-e3c033dd0b2c`; QA independently confirmed its unsigned endpoint returns `401 UNAUTHORIZED` on 2026-07-21.
- [x] Source migration `0024` serializes provider telemetry per request using a transaction-scoped advisory lock and preserves the maximum count atomically. Hosted QA source submits concurrent counts 3 and 2 and expects one summary at 3 plus attempts 1–3; focused contracts pass.
- [ ] Run the new provider-count concurrency case against deployed staging migration `0024`; Technical reports deployment parity, but source assertions and migration-list parity are not a real Postgres concurrency pass.
...
- [x] Source migration `0025` exposes service-role-only scheduler status without cron commands/secrets and checks the exact three schedules plus their latest run results; focused contracts pass. Technical reports `0025` deployed.
- [ ] Run the privileged suite and capture `runtime_scheduler_status()` evidence that all three jobs are active, correctly scheduled, and have a latest successful execution. Deployment alone does not prove cron execution.
...
### Claude Fable Auth and System States handoff (`2026-07-23`)

- [x] Source builds the shared Auth/System States kit: Magic Link Sent, Restoring Session, Auth Failure, Restoring Space, No Chart Found, Offline Banner, Skeleton Rows, System Retry Card, and Permission Bridge. The existing sign-in screen remains AUTH-001.
- [x] Live source wires successful magic-link sends to the inbox state and signed-in manual reload through Restoring Space to Chat or No Chart Found. Workspace typecheck and `test:mobile-ui` independently pass.
- [ ] Fix resend truthfulness before device signoff. The UI immediately changes to `Link resent` and disables the control, while `resendLink` catches and hides delivery/rate-limit errors; only show success after the async send actually succeeds and expose a retryable failure.
- [ ] Verify the displayed “expires in 15 minutes” value against the actual Supabase magic-link configuration, or replace it with non-numeric copy that cannot drift.
- [ ] Fix No Chart Found `Sign out`: its current callback only navigates to the Auth screen and does not sign the user out. The action must either perform real sign-out/visible-state cleanup or be relabelled accurately.
- [ ] Make `restoreSpace` failures fully retryable: `refreshAuthStatus()` currently runs before the `try`, so an auth-refresh/network exception can escape without setting the failed state.
- [ ] Add dedicated Auth/System State contract tests. The current general mobile UI contract passing does not verify resend failure, sign-out behavior, restore failure, timer cleanup, reduced motion, or the individual unwired states.
- [ ] Keep Restoring Session, Auth Failure, Offline Banner, Skeleton Rows, System Retry Card, and Permission Bridge marked source-only until their real deep-link, network, loading/error, camera, motion, and notification triggers are wired and device-tested.
- [ ] PM decision: PROF-2 birth-detail regeneration must be backend-enforced and should be closed soon. The current uncommitted `App.tsx` local fallback/counter must not be presented as PROF-2, must not ship in staging/production as authoritative behavior, and must be removed/disabled or clearly guarded as dev-preview only. Technical should implement/verify the server-owned transaction, three-change counter, approved errors, chart regeneration, hosted tests, and device restore flow before QA marks PROF-2 complete.
...
- [ ] Independently visually verify the celestial background at 390×844 against the Claude reference. The seeded 66 positions and static visual layers remain, but commit `9f2a59f` intentionally replaced per-star opacity/duration/delay animation with two grouped native pulses for performance. Confirm the grouped effect still meets the approved design before closing visual fidelity.
...
- [x] QA independently confirmed on 2026-07-21 that staging `/profile`, `/chat-message`, `/account-deletion-request`, `/external-sync-retry`, and the chart Worker all return HTTP 401 for unauthenticated/unsigned POSTs. Technical reports Worker version `def65b99-6a3f-429f-800a-e3c033dd0b2c`; signed timeout/replay/timezone behavior and the exact deployed migration set still require privileged evidence.
...
- [ ] PROF-2 source exists in migration `0026` and the Edge/mobile integration, with three-change enforcement, error codes, transactional version activation, and prior-chart rollback semantics. Close only after the retry/routing/validation fixes above, migration/function deployment, and privileged hosted PostgreSQL/device tests.
