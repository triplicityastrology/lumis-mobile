...
Status: source-complete, inert, and unrun. These packets do not authorize or
...
Each apply packet has a matching `.rehearsal.sql` file in the same directory.

Each packet is checksum-bound to its reviewed migration and contains that
migration body plus its exact migration-history insert inside one transaction.
The preflight accepts only the T82-confirmed `version text`, `statements text[]`,
and `name text` history shape and the exact preceding version/name sequence.
Any schema or parity difference raises `S2_T40_STOP_HISTORY_SHAPE_MISMATCH` or
`S2_T40_STOP_REMOTE_PARITY_MISMATCH` before the migration body.
The apply artifact ends in `commit;`; its independently reviewable rehearsal
counterpart is identical except that it ends in `rollback;`.
...
T82 confirmed the exact history shape and version/name parity. Before a future
write, revalidate the same redacted envelope locally; do not rerun or broaden
the Dashboard query unless PM separately authorizes it. A changed shape or
parity result stops all packets.

### Safe rollback rehearsal

Run each checked-in rehearsal in exact `0032`, `0033`, `0034` order during a
separately authorized window. After each rollback, confirm with metadata-only
reads that neither its history version nor schema changes persisted. Stop if a
rehearsal errors, writes survive rollback, or the observed order/shape differs.
Do not run the corresponding apply packet until PM/QA accepts that rehearsal.
...
Execute no packet from this commit. Source completeness is not write authority.
