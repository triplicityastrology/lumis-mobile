...
// Real keys derived from the resolution (sel.p = [pt(theme), px(context), ...p01..related]; sel.e = e01..).
const P1 = sel.p[2], P2 = sel.p[3], P3 = sel.p[4];   // three distinct planet related codes (p01,p02,p03)
const H1 = sel.h[2], H2 = sel.h[3], H3 = sel.h[4];   // three distinct house related codes (h01,h02,h03)
const E1 = sel.e[0];                                  // an element code (e01)
ok(/^p\d\d$/u.test(P1) && /^h\d\d$/u.test(H1) && /^e\d\d$/u.test(E1), "compact wire codes present");
...
eq(res.gid["pt"], "planet.moon.theme", "pt -> planet.moon.theme");
eq(res.gid["ht"], "house.4.setting", "ht -> house.4.setting");
// Rebuilding (any language) yields identical code->gid pairs — codes do not depend on list order.
 ...
 * The lossless compact wire encoding serialises related/element places as a { code: text } map,
 * so completeness is checked on the map's key count (== the bank), and theme/context are retained. */
for (const planet of DICE_V05_PLANET_IDS) {
  ...
  eq(Object.keys(pp.related).length, LOCATION_PLANET_BANK[planet].related.length, `planet ${planet} related count == bank`);
  ok(Object.keys(pp.related).length >= 5, `planet ${planet} has >=5 related places (not summarised)`);
  ok(typeof pp.theme?.t === "string" && typeof pp.context?.t === "string", `planet ${planet} theme + context hints retained`);
...
for (let h = 1; h <= 12; h++) {
  ...
  eq(Object.keys(hp.related).length, LOCATION_HOUSE_BANK[h].related.length, `house ${h} related count == bank`);
  ok(["near", "middle", "far"].includes(hp.distance), `house ${h} fixed distance present`);
  ok(typeof hp.setting?.t === "string" && typeof hp.context?.t === "string", `house ${h} setting + context hints retained`);
...
for (const sign of DICE_V05_SIGN_IDS) {
  ...
  eq(Object.keys(se.places).length, ELEMENT_TABLE[el].places.length, `sign ${sign} (${el}) element place count`);
  ok(typeof se.direction === "string", `sign ${sign} element direction retained`);
